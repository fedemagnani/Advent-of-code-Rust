...
fn solve_day_6(){
    ...
    println!("Sum of questions answered affermatively by each member of a given group: {}",result_2);
}

fn solve_day_7(){
    let file_name="inputDay7.txt";
    let result = advent_of_code::day7::part1(&file_name).unwrap();
    println!("Shiny gold bag can be contained in {} types of bags", result);
...
fn solve_day_8(){
    ...
    println!("At the end of the first cycle, the accumulator lead to {}", result);
}